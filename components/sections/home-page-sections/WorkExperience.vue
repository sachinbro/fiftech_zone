<template>
    <v-row class="mt-5 mt-sm-16 background">
        <v-col cols="12" sm="6">
            <v-row class="text-h6 text-sm-h4  font-weight-bold ma-1 ma-sm-10">
                <v-container>
                    <div>We listen and work</div>
                    <div>together to create</div>
                    <div>a truly unique experience.</div>
                    <div class="text-caption mr-14 text--secondary mt-2 mt-sm-6">This creates a work environment of trust. Having the trust of your employees gives you greater influence over them. At the same time, it makes them more motivated and committed to their work.</div>
                    <v-row>
                        <v-col cols="1" class="ml-n6 ml-md-0"><img src="../../../static/sections/Homepage/Image 2.png" height="70px" width="60px"></v-col>
                        <v-col sm="7" class="mt-sm-4 mt-2 ml-6 ml-sm-2">
                            <div class="text-h6">Digital expericence</div>
                            <div class="text-caption">User interaction with good digital touch-point, implies providing a fast, responsive and frictionless experience for the client.</div>
                            
                        </v-col>
                        <v-col cols="0" sm="2"></v-col>
                    </v-row>
                    <v-row>
                        <v-col cols="1" class="ml-n6 ml-md-0" ><img src="../../../static/sections/Homepage/Image 3.png" height="70px" width="60px"></v-col>
                        <v-col sm="7" class="mt-sm-4 mt-4 ml-6 ml-sm-2">    
                            <div class="text-h6">Branding strategy</div>
                            <div class="text-caption">The long term plan to achieve a series of long term goals that ultimately result in the identification.</div>
                            
                        </v-col>
                        <v-col cols="0" sm="2" ></v-col>
                    </v-row>
                </v-container>
            </v-row>
        </v-col>
        <v-col class="d-flex align-center justify-center" cols="12" md="6" >
            <img src="../../../static/sections/Homepage/Group 319.png" class="hidden-sm-and-down" width="500px" height="350px" alt="">
            <img src="../../../static/sections/Homepage/Group 319.png" class="hidden-md-and-up" width="300px" height="200px" alt="">
        </v-col>
    </v-row>
</template>
<style scoped>
.background{
    background-image: url("../../../static/sections/Homepage/image11.png");
    background-repeat: no-repeat;
    height: 80vh;

    background-size: 30% 100%;
    background-position: left;
}
</style>